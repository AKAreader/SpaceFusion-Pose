...
class PoseDataset(Dataset):
    ...
    新增：
      ...
      roi_expand_min/max: only used when random
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "ir":
            ...
            # crop RGB: share same box by default
            if self.share_crop:
                rgb_roi = _crop(rgb, box)
            else:
                # fallback (not recommended): separate sampling
                expand2 = self._sample_expand()
                jitter2 = self.roi_jitter if self.train else 0.0
                box2 = _sample_crop_box(roi, H=rgb.shape[0], W=rgb.shape[1],
                                        expand=expand2, jitter=jitter2, rng=self.rng)
                rgb_roi = _crop(rgb, box2)
