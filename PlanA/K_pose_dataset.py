...
def _sample_crop_box(roi_xyxy, H, W, expand, jitter, rng: np.random.RandomState):
    """
    roi_xyxy: [x1,y1,x2,y2] in original image coords
    expand: scale factor on box size
    jitter: relative shift ratio (0.1 means shift up to 10% of box size)
    """
    ...
    # jitter (same box used for both IR/RGB in rgbir mode)
    if jitter > 0:
...
def _get_K_full(item: dict) -> np.ndarray:
    """
    item["camera"] is expected:
      {"fx":..., "fy":..., "cx":..., "cy":...} in pixel units (for full 1920x1080).
    """
    ...
    if fx <= 0 or fy <= 0:
        # fallback: assume principal point at center and fx,fy from image size (very rough)
...
def _K_after_crop_and_resize(K_full: np.ndarray, box_xyxy, out_size: int) -> np.ndarray:
    """
    Convert K_full (full image) -> K_out (for cropped+resized square image)
    """
    x1, y1, x2, y2 = box_xyxy
    crop_w = float(max(1, x2 - x1))
    crop_h = float(max(1, y2 - y1))

    sx = float(out_size) / crop_w
    sy = float(out_size) / crop_h

    fx = float(K_full[0, 0]) * sx
    fy = float(K_full[1, 1]) * sy
    cx = (float(K_full[0, 2]) - float(x1)) * sx
    cy = (float(K_full[1, 2]) - float(y1)) * sy

    K_out = np.array([[fx, 0.0, cx],
                      [0.0, fy, cy],
                      [0.0, 0.0, 1.0]], dtype=np.float32)
    return K_out
...
class PoseDataset(Dataset):
    """
    mode:
      - "ir":    only IR ROI -> [1,H,W]
      - "rgbir": RGB(3)+IR(1) -> [4,H,W]

    roi_expand_mode: "fixed" or "random"
    share_crop: in rgbir, enforce SAME crop box for RGB & IR (recommended)
    return_camera:
      - False: return (x, q_gt, id)  [保持兼容 test_pose.py]
      - True : return (x, q_gt, t_gt, K_out, box_xyxy, id)
    """
    def __init__(self,
                 data_root: str,
                 json_path: str,
    ...
    def __getitem__(self, idx):
        ...
        ir_roi = _norm_ir_default(ir_roi)  # HxW
        ...
        if self.mode == "ir":
            ...
            if not self.share_crop:
                # 强烈不建议：rgb/ir 裁不同 box 会让 4 通道不对齐
                # 这里直接强制共享，避免“训练对齐错误”
                pass
        ...
        q_gt_t = torch.from_numpy(q_gt)

        if not self.return_camera:
            return x, q_gt_t, item["id"]

        # ===== camera extras for geometry =====
        K_full = _get_K_full(item)  # pixel K for full 1920x1080
        K_out = _K_after_crop_and_resize(K_full, box, self.out_size)  # pixel K for out_size ROI image
        t_gt = np.array(item.get("position_xyz_m", [0.0, 0.0, 0.0]), dtype=np.float32)
        ...
        return (
            ...
            torch.tensor(box, dtype=torch.int32),
            item["id"],
        )
