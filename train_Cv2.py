...
# -*- encoding: utf-8 -*-
"""
train_C_v2.py  （建议你保存为这个新文件，不会覆盖旧的 trainB/trainC 结果）

目标：
在保持 C 的姿态优势（angle/p95 不差）的同时，显式提升融合结构保真（SSIM），避免“过锐但结构被破坏”。

你之前“C 关 Rectify 指标全面变好”的现象，强烈暗示：
- Rectify 在你的数据/实现下容易伤结构（SSIM）甚至干扰姿态；
- 或者训练/推理阶段 Rectify 实现不一致（推理用了有参数 rectify 子网但没训练到权重）。

因此本文件做两件事：
1) Rectify 默认更保守：并且默认关闭（USE_SAL_RECTIFY=0）。你需要时再开。
2) 增加可微 SSIM 损失：训练目标里显式奖励结构相似性，并做 warmup/ramp。
...
POSE_INPUT_RGB=1/0
LR_POSE（默认 2e-4）
LAMBDA_POSE（最大权重，默认 0.08）
POSE_START_EPOCH（默认 3）
POSE_RAMP_EPOCHS（默认 4）
POSE_GRAD_CLIP（默认 1.0）

Rectify：
USE_SAL_RECTIFY=1/0  （默认 0：更稳）
RECTIFY_ALPHA_MAX（默认 0.18）
RECTIFY_START_EPOCH（默认 5）
RECTIFY_RAMP_EPOCHS（默认 6）
RECTIFY_DETACH_SAL=1/0

SSIM：
USE_SSIM_LOSS=1/0 （默认 1）
LAMBDA_SSIM（默认 0.14）
SSIM_START_EPOCH（默认 5）
SSIM_RAMP_EPOCHS（默认 4）
SSIM_WIN（默认 11）
SSIM_SIGMA（默认 1.5）
...
SAFE_RUNTIME = os.environ.get("SAFE_RUNTIME", "0") == "1"
...
class CFG:
    ...
    # Gate 防塌陷/平滑（更“稳”，减少门控斑驳，提升 SSIM）
    LAMBDA_TV: float = float(os.environ.get("LAMBDA_TV", "0.02"))
    LAMBDA_GMEAN: float = float(os.environ.get("LAMBDA_GMEAN", "0.00"))
    G_MIN: float = float(os.environ.get("G_MIN", "0.15"))
    ...
    # Pose 权重调度
    LAMBDA_POSE: float = float(os.environ.get("LAMBDA_POSE", "0.08"))
    ...
    POSE_LOSS_TO_TOTAL: bool = os.environ.get("POSE_LOSS_TO_TOTAL", "1") == "1"

    # 可选：整体梯度裁剪
    MODEL_GRAD_CLIP: float = float(os.environ.get("MODEL_GRAD_CLIP", "0"))  # 建议 3.0

    # Pose CA
    USE_CA_POSE: bool = os.environ.get("USE_CA_POSE", "1") == "1"
    CA_REDUCTION: int = int(os.environ.get("CA_REDUCTION", "32"))

    # Rectify（默认更保守：默认关闭；需要时再开）
    USE_SAL_RECTIFY: bool = os.environ.get("USE_SAL_RECTIFY", "0") == "1"
    RECTIFY_ALPHA_MAX: float = float(os.environ.get("RECTIFY_ALPHA_MAX", "0.18"))
    RECTIFY_START_EPOCH: int = int(os.environ.get("RECTIFY_START_EPOCH", "5"))
    RECTIFY_RAMP_EPOCHS: int = int(os.environ.get("RECTIFY_RAMP_EPOCHS", "6"))
    RECTIFY_DETACH_SAL: bool = os.environ.get("RECTIFY_DETACH_SAL", "1") == "1"

    # SSIM loss（显式提升结构保真）
    USE_SSIM_LOSS: bool = os.environ.get("USE_SSIM_LOSS", "1") == "1"
    LAMBDA_SSIM: float = float(os.environ.get("LAMBDA_SSIM", "0.14"))
    SSIM_START_EPOCH: int = int(os.environ.get("SSIM_START_EPOCH", "5"))
    SSIM_RAMP_EPOCHS: int = int(os.environ.get("SSIM_RAMP_EPOCHS", "4"))
    SSIM_WIN: int = int(os.environ.get("SSIM_WIN", "11"))
    SSIM_SIGMA: float = float(os.environ.get("SSIM_SIGMA", "1.5"))

    # init ckpt（默认用 B 的 best_angle；也可用环境变量覆盖）
    INIT_FUSION_CKPT: str = os.environ.get(
        "INIT_FUSION_CKPT",
        r"D:\Redundancy\edgedownload\SeAFusion-main\runs_fusion\routeB_gate\20260106-170140\ckpt\fusion_best_angle.pth"
    ).strip()

    # out
    RUNS_ROOT: str = os.environ.get("RUNS_ROOT", r"./runs_fusion")
    TAG: str = os.environ.get("TAG", "routeC_v2_ssim").strip()
    CUDNN_BENCHMARK: bool = os.environ.get("CUDNN_BENCHMARK", "1") == "1"
    CUDNN_DETERMINISTIC: bool = os.environ.get("CUDNN_DETERMINISTIC", "0") == "1"
    DEBUG_FIRST_BATCH: bool = os.environ.get("DEBUG_FIRST_BATCH", "0") == "1"
    DEBUG_ONE_BATCH_ONLY: bool = os.environ.get("DEBUG_ONE_BATCH_ONLY", "0") == "1"


def _validate_cfg(cfg: CFG):
    # SSIM win 必须为奇数且 >= 3
    ...
    if cfg.SSIM_WIN % 2 == 0:
        cfg.SSIM_WIN += 1
    if cfg.RECTIFY_ALPHA_MAX < 0:
        cfg.RECTIFY_ALPHA_MAX = 0.0
    if cfg.G_MIN < 0:
        cfg.G_MIN = 0.0
    if cfg.G_MIN > 0.95:
        cfg.G_MIN = 0.95
...
def _try_load_state_dict_stripped(model: nn.Module, sd: dict, logger: logging.Logger) -> None:
    ...
    if any(k.startswith("module.") for k in keys):
        ...
        keys = list(sd.keys())
    # 如果 sd 是 FusionWithGate 的 state_dict，会有 fusion.xxx / gate.xxx
    if any(k.startswith("fusion.") for k in keys):
        sd = {k.replace("fusion.", "", 1): v for k, v in sd.items() if k.startswith("fusion.")}
...
# -------------------------
# 7.1) 可微 SSIM（用于训练）+ kernel cache
# -------------------------
_SSIM_KERNEL_CACHE: Dict[tuple, torch.Tensor] = {}

def _gaussian_kernel_2d(win: int, sigma: float, device, dtype):
    ...
    g = g / (g.sum() + 1e-12)
    k2d = (g[:, None] * g[None, :]).unsqueeze(0).unsqueeze(0)  # [1,1,win,win]
    _SSIM_KERNEL_CACHE[key] = k2d
    return k2d

def ssim_torch(x: torch.Tensor, y: torch.Tensor, win: int = 11, sigma: float = 1.5) -> torch.Tensor:
    """
    x,y: [B,1,H,W] in [0,1]
    return: scalar SSIM (mean)
    """
    x = x.clamp(0, 1)
    y = y.clamp(0, 1)
    device, dtype = x.device, x.dtype
    k = _gaussian_kernel_2d(win, sigma, device, dtype)
    pad = win // 2
    ...
    sig_x2 = F.conv2d(x * x, k, padding=pad) - mu_x2
    sig_y2 = F.conv2d(y * y, k, padding=pad) - mu_y2
    ...
    den = (mu_x2 + mu_y2 + C1) * (sig_x2 + sig_y2 + C2) + 1e-12
    ssim_map = num / den
    return ssim_map.mean()

def ssim_fusion_loss(fused: torch.Tensor, Y: torch.Tensor, IR: torch.Tensor, win: int, sigma: float) -> torch.Tensor:
    s1 = ssim_torch(fused, Y, win=win, sigma=sigma)
    s2 = ssim_torch(fused, IR, win=win, sigma=sigma)
    return 1.0 - 0.5 * (s1 + s2)
...
def train():
    ...
    logger.info("LOSS_MODE: routeC_v2 (routeA_v2 + gate + (optional)rectify + pose + SSIM)")
    ...
    logger.info(
        ...
        f"lr_pose={cfg.LR_POSE}, grad_clip={cfg.POSE_GRAD_CLIP}, to_total={cfg.POSE_LOSS_TO_TOTAL}"
    )
    logger.info(
        f"SSIM_SCHED      : use={cfg.USE_SSIM_LOSS}, lambda_max={cfg.LAMBDA_SSIM}, start={cfg.SSIM_START_EPOCH}, "
        f"ramp={cfg.SSIM_RAMP_EPOCHS}, win={cfg.SSIM_WIN}, sigma={cfg.SSIM_SIGMA}"
    )
    logger.info(f"INIT_FUSION_CKPT: {cfg.INIT_FUSION_CKPT}")
    logger.info(f"DEBUG_FIRST_BATCH: {cfg.DEBUG_FIRST_BATCH} | DEBUG_ONE_BATCH_ONLY: {cfg.DEBUG_ONE_BATCH_ONLY}")
    runtime_num_workers = 0 if cfg.SAFE_RUNTIME else cfg.NUM_WORKERS
    runtime_pin_memory = False if cfg.SAFE_RUNTIME else cfg.PIN_MEMORY
    runtime_non_blocking = False if cfg.SAFE_RUNTIME else cfg.NON_BLOCKING
    runtime_accum_steps = cfg.ACCUM_STEPS
    runtime_cudnn_benchmark = False if cfg.SAFE_RUNTIME else cfg.CUDNN_BENCHMARK
    runtime_cudnn_deterministic = True if cfg.SAFE_RUNTIME else cfg.CUDNN_DETERMINISTIC
    logger.info(
        f"RUNTIME          : safe_runtime={cfg.SAFE_RUNTIME}, num_workers={runtime_num_workers}, "
        f"pin_memory={runtime_pin_memory}, non_blocking={runtime_non_blocking}, accum_steps={runtime_accum_steps}, "
        f"cudnn.benchmark={runtime_cudnn_benchmark}, cudnn.deterministic={runtime_cudnn_deterministic}"
    )

    torch.backends.cudnn.benchmark = runtime_cudnn_benchmark
    torch.backends.cudnn.deterministic = runtime_cudnn_deterministic
    ...
    # DataLoader（如 worker 退出，先把 NUM_WORKERS=0）
    train_loader = DataLoader(
        train_ds, batch_size=cfg.BATCH_SIZE, shuffle=True, num_workers=runtime_num_workers,
        pin_memory=runtime_pin_memory, drop_last=True, persistent_workers=(runtime_num_workers > 0),
    )
    val_loader = DataLoader(
        val_ds, batch_size=max(1, cfg.BATCH_SIZE // 2), shuffle=False, num_workers=runtime_num_workers,
        pin_memory=runtime_pin_memory, drop_last=False, persistent_workers=(runtime_num_workers > 0),
    )
    ...
    for epoch in range(1, cfg.EPOCHS + 1):
        ...
        # SSIM schedule（渐进）
        ssim_start = max(cfg.SSIM_START_EPOCH, cfg.GATE_WARMUP_EPOCHS + 1)
        ssim_w = 0.0
        if cfg.USE_SSIM_LOSS:
            ssim_w = ramp_weight(epoch, ssim_start, cfg.SSIM_RAMP_EPOCHS, cfg.LAMBDA_SSIM)

        logger.info(f"[SCHED] epoch={epoch} alpha={alpha:.2f} rect_alpha={rect_alpha:.3f} pose_w={pose_w:.3f} ssim_w={ssim_w:.3f}")

        st = time.time()
        meters = {"tot": [], "fus": [], "Lin": [], "Lgrad": [], "hit": [], "pose": [], "ssim": [], "gmean": [], "gstd": [], "bypass": []}
        ...
        for step_idx, (vis, ir, _pose_dummy, _mask_dummy, names) in enumerate(pbar):
            first_iter = (epoch == 1 and step_idx == 0)
            if cfg.DEBUG_FIRST_BATCH and first_iter:
                logger.info(
                    f"[DEBUG] first batch (CPU) vis shape={tuple(vis.shape)} device={vis.device} | "
                    f"ir shape={tuple(ir.shape)} device={ir.device}"
                )
            vis = vis.to(device, non_blocking=runtime_non_blocking)
            ir  = ir.to(device, non_blocking=runtime_non_blocking)
            if cfg.DEBUG_FIRST_BATCH and first_iter:
                logger.info(
                    f"[DEBUG] first batch (CUDA) vis shape={tuple(vis.shape)} device={vis.device} | "
                    f"ir shape={tuple(ir.shape)} device={ir.device}"
                )
            ...
            if cfg.DEBUG_FIRST_BATCH and first_iter:
                logger.info(
                    f"[DEBUG] before first forward: Y shape={tuple(Y.shape)} device={Y.device} | "
                    f"sal shape={tuple(sal.shape)} device={sal.device}"
                )

            fused_Y, g, base = model(Y, ir)
            if cfg.DEBUG_FIRST_BATCH and first_iter:
                logger.info(
                    f"[DEBUG] after first forward: fused_Y shape={tuple(fused_Y.shape)} device={fused_Y.device} | "
                    f"g shape={tuple(g.shape)} device={g.device} | base shape={tuple(base.shape)} device={base.device}"
                )
            ...
            # Rectify（可选）：warmup 后 & 按 rect_alpha 渐进
            ...
            L_ssim = None
            if ssim_w > 0:
                L_ssim = ssim_fusion_loss(fused_Y, Y, ir, win=cfg.SSIM_WIN, sigma=cfg.SSIM_SIGMA)
                total = total + ssim_w * L_ssim
            ...
            if use_pose and pose_net and (pose_w > 0):
                ...
                if int(valid.sum().item()) > 0:
                    ...
                    if cfg.POSE_LOSS_TO_TOTAL:
                        total = total + pose_w * pose_loss_val

            if cfg.DEBUG_FIRST_BATCH and first_iter:
                logger.info(
                    f"[DEBUG] before backward: total={float(total.detach().cpu()):.6f} "
                    f"device={total.device} requires_grad={total.requires_grad}"
                )
            (total / float(runtime_accum_steps)).backward()
            should_step = (((step_idx + 1) % runtime_accum_steps) == 0) or ((step_idx + 1) == num_train_batches)
            if cfg.DEBUG_ONE_BATCH_ONLY and first_iter:
                should_step = True
            ...
            if cfg.DEBUG_ONE_BATCH_ONLY and first_iter:
                logger.info("[DEBUG] DEBUG_ONE_BATCH_ONLY=1 -> stop after first training batch.")
                return
            ...
            if L_ssim is not None:
                meters["ssim"].append(float(L_ssim.detach().cpu()))
            ...
            if meters["ssim"]:
                msg += f" | ssimL={np.mean(meters['ssim']):.4f}"
        ...
        val_tot, val_fus, val_pose, val_hit, val_ssim = [], [], [], [], []
        all_angles = []
        ...
        with torch.no_grad():
            for (vis, ir, _pose_dummy, _mask_dummy, names) in val_loader:
                ...
                if ssim_w > 0:
                    L_ssim = ssim_fusion_loss(fused_Y, Y, ir, win=cfg.SSIM_WIN, sigma=cfg.SSIM_SIGMA)
                    total = total + ssim_w * L_ssim
                    val_ssim.append(float(L_ssim.cpu()))
        ...
        train_ssim = float(np.mean(meters["ssim"])) if meters["ssim"] else float("nan")
        ...
        v_ssim = float(np.mean(val_ssim)) if val_ssim else float("nan")

        el = time.time() - st
        logger.info(
            f"[Epoch {epoch}] alpha={alpha:.2f} rect_alpha={rect_alpha:.3f} pose_w={pose_w:.3f} ssim_w={ssim_w:.3f} "
            f"train_total={train_total:.4f} train_fusion={train_fusion:.4f} train_pose={train_pose} train_hit={train_hit:.3f} train_ssimL={train_ssim} | "
            f"val_total={v_total:.4f} val_fusion={v_fusion:.4f} val_pose={v_pose} val_hit={v_hit:.3f} val_ssimL={v_ssim} | "
...
def debug_forward_one_batch():
    cfg = CFG()
    _validate_cfg(cfg)

    debug_device = os.environ.get("DEBUG_DEVICE", "cpu").strip().lower()
    if debug_device == "cuda":
        if not torch.cuda.is_available():
            raise RuntimeError("DEBUG_DEVICE=cuda but CUDA is not available.")
    ...
    print(f"[FORWARD_DEBUG] device={device}")
    print(f"[FORWARD_DEBUG] cudnn.benchmark={torch.backends.cudnn.benchmark}")
    print(f"[FORWARD_DEBUG] cudnn.deterministic={torch.backends.cudnn.deterministic}")

    ds = PairFolderDataset(cfg.RGB_DIR, cfg.IR_DIR, "train", cfg.RESIZE_HW, cfg.SPLIT_SEED, cfg.TRAIN_RATIO)
    loader = DataLoader(
        ds,
        batch_size=1,
        shuffle=False,
    ...
    model = FusionWithGate(g_min=cfg.G_MIN).to(device)
    logger = logging.getLogger("forward_debug")
    ...
    vis, ir, _pose_dummy, _mask_dummy, names = next(iter(loader))
    print(f"[FORWARD_DEBUG] sample={names[0]}")
    print(f"[FORWARD_DEBUG] CPU vis shape={tuple(vis.shape)} dtype={vis.dtype} device={vis.device}")
    print(f"[FORWARD_DEBUG] CPU ir  shape={tuple(ir.shape)} dtype={ir.dtype} device={ir.device}")

    vis = vis.to(device, non_blocking=False)
    ir = ir.to(device, non_blocking=False)
    print(f"[FORWARD_DEBUG] DEV vis shape={tuple(vis.shape)} dtype={vis.dtype} device={vis.device}")
    print(f"[FORWARD_DEBUG] DEV ir  shape={tuple(ir.shape)} dtype={ir.dtype} device={ir.device}")

    Y, Cb, Cr = RGB2YCrCb(vis)
    sal = joint_saliency(Y, ir, cfg.SAL_ROI_EXPAND_K, cfg.SAL_THRESH)
    print(f"[FORWARD_DEBUG] pre-forward Y   shape={tuple(Y.shape)} dtype={Y.dtype} device={Y.device}")
    print(f"[FORWARD_DEBUG] pre-forward Cb  shape={tuple(Cb.shape)} dtype={Cb.dtype} device={Cb.device}")
    print(f"[FORWARD_DEBUG] pre-forward Cr  shape={tuple(Cr.shape)} dtype={Cr.dtype} device={Cr.device}")
    print(f"[FORWARD_DEBUG] pre-forward sal shape={tuple(sal.shape)} dtype={sal.dtype} device={sal.device}")
    print("[FORWARD_DEBUG] before model(Y, ir)")

    with torch.no_grad():
        fused_Y, g, base = model(Y, ir)

    print("[FORWARD_DEBUG] after model(Y, ir)")
    print(f"[FORWARD_DEBUG] post-forward fused_Y shape={tuple(fused_Y.shape)} dtype={fused_Y.dtype} device={fused_Y.device}")
    print(f"[FORWARD_DEBUG] post-forward g       shape={tuple(g.shape)} dtype={g.dtype} device={g.device}")
    print(f"[FORWARD_DEBUG] post-forward base    shape={tuple(base.shape)} dtype={base.dtype} device={base.device}")
...
if __name__ == "__main__":
    ...
    if os.environ.get("FORWARD_DEBUG_ONLY", "0") == "1":
        debug_forward_one_batch()
    else:
        train()
